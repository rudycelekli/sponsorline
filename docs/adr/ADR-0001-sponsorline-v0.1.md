...
**Self-learning hook (Playbook differentiator):** relevance ranking within the eligible set uses an agentdb-style `SolverBandit` (Thompson sampling) keyed by interest-vector bucket, rewarded by a *local, privacy-safe* signal (e.g., dev did not run `off`/`why`-dismiss after the impression). The bandit state is local and serializable; no behavioral data leaves the device. This makes the line get more relevant with use without surveillance.
